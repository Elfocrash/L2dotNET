    ...
    public class PlayerRepository : IPlayerRepository
    {
        ...
        public void CreatePlayer(PlayerModel player)
        {
            this.db.Execute(@"insert into characters (account_name, obj_Id, char_name, Level, MaxHp, CurHp, MaxCp, CurCp,MaxMp,CurMp,
               ...
               @Face,@HairStyle,@HairColor,@Sex,@Heading,@X,@Y,@Z,@Exp,@ExpBeforeDeath,@Sp,@Karma,@PvpKills,@PkKills,@ClanId,@Race,@classid,@base_class, @DeleteTime,@CanCraft,@Title,
                ...
                CanCraft = player.CanCraft, Title = player.Title, rec_have = player.RecHave, rec_left = player.RecLeft, AccessLevel = player.AccessLevel, char_slot = player.CharSlot, clan_privs = player.ClanPrivs,
        ...
        public void UpdatePlayer(PlayerModel player)
        {
            ...
            death_penalty_level=@death_penalty_level WHERE obj_id=@obj_id", new {
                level = player.Level, maxHp = player.MaxHp, curHp = player.CurHp, maxCp = player.MaxCp, curCp = player.CurCp,
                maxMp = player.MaxMp, curMp = player.CurMp, face = player.Face, hairStyle = player.HairStyle, hairColor = player.HairColor, sex = player.Sex, heading = player.Heading,
                x = player.X, y = player.Y, z = player.Z, exp = player.Exp, expBeforeDeath = player.ExpBeforeDeath, sp = player.Sp, karma = player.Karma,
                pvpkills = player.PvpKills, pkkills = player.PkKills, clanid = player.ClanId, online = player.Online, onlinetime = player.OnlineTime,
                race = player.Race, classid = player.ClassId, base_class = player.BaseClass, deletetime = player.DeleteTime, canCraft = player.CanCraft, title = player.Title,
                rec_have = player.RecHave, rec_left = player.RecLeft, accesslevel = player.AccessLevel, clan_privs = player.ClanPrivs, wantspeace = player.WantsPeace,
                isin7sdungeon = player.IsIn7sDungeon, punish_level = player.PunishLevel, punish_timer = player.PunishTimer, power_grade = player.PowerGrade, nobless = player.Nobless,
                subpledge = player.Subpledge, last_recom_date = player.LastRecomDate,lvl_joined_academy = player.LevelJoinedAcademy, apprentice = player.Apprentice,
                sponsor = player.Sponsor, varka_ketra_ally = player.VarkaKetraAlly, clan_join_expiry_time = player.ClanJoinExpiryTime, clan_create_expiry_time = player.ClanCreateExpiryTime,
                death_penalty_level = player.DeathPenaltyLevel, obj_Id = player.ObjectId
            });
        }

...
namespace L2dotNET.Game.model.quests.data
{
    ...
    {
        const int jasmine = 30134;
        const int sentry_roseline = 30355;
        const int harne = 30144;

        const int q_roseline_paper = 7573;
        ...
        public _0008_an_adventure_begins()
        {
            ...
            startNpc = jasmine;
            talkNpcs = new int[] { startNpc, sentry_roseline, harne };
            actItems = new int[] { q_roseline_paper };
        }

        public override void tryAccept(L2Player player, L2Citizen npc)
        {
            if (player.BaseClass.race == RaceId.darkelf && player.Level >= 3)

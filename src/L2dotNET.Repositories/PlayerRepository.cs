    ...
    public class PlayerRepository : IPlayerRepository
    {
        ...
        public PlayerModel GetAccountByLogin(int objId)
        {
            try
            {
                ...
                DeathPenaltyLevel from characters where obj_Id=@id", new { id = objId }).FirstOrDefault();
            }
        ...
        public void CreatePlayer(PlayerModel player)
        {
            try
            {
                db.Execute(@"insert into characters (account_name, obj_Id, char_name, Level, MaxHp, CurHp, MaxCp, CurCp,MaxMp,CurMp,
                                ...
                                @Face,@HairStyle,@HairColor,@Sex,@Heading,@X,@Y,@Z,@Exp,@ExpBeforeDeath,@Sp,@Karma,@PvpKills,@PkKills,@ClanId,@Race,@classid,@base_class, @DeleteTime,@CanCraft,@Title,
                                @rec_have,@rec_left,@AccessLevel,@char_slot,@clan_privs, @WantsPeace,@IsIn7sDungeon,@punish_level,@punish_timer, @power_grade,@Nobless,@Hero,@Subpledge,
                                @last_recom_date,@lvl_joined_academy, @Apprentice, @Sponsor,@varka_ketra_ally,@clan_join_expiry_time,@clan_create_expiry_time, @death_penalty_level)", new { account_name = player.AccountName, obj_Id = player.ObjectId, char_name = player.Name, Level = player.Level, MaxHp = player.MaxHp, CurHp = player.CurHp, MaxCp = player.MaxCp, CurCp = player.CurCp, MaxMp = player.MaxMp, CurMp = player.CurMp, Face = player.Face, HairStyle = player.HairStyle, HairColor = player.HairColor, Sex = player.Sex, Heading = player.Heading, X = player.X, Y = player.Y, Z = player.Z, Exp = player.Exp, ExpBeforeDeath = player.ExpBeforeDeath, Sp = player.Sp, Karma = player.Karma, PvpKills = player.PvpKills, PkKills = player.PkKills, ClanId = player.ClanId, Race = player.Race, classid = player.ClassId, base_class = player.BaseClass, DeleteTime = player.DeleteTime, CanCraft = player.CanCraft, Title = player.Title, rec_have = player.RecHave, rec_left = player.RecLeft, AccessLevel = player.AccessLevel, char_slot = player.CharSlot, clan_privs = player.ClanPrivs, WantsPeace = player.WantsPeace, IsIn7sDungeon = player.IsIn7sDungeon, punish_level = player.PunishLevel, punish_timer = player.PunishTimer, power_grade = player.PowerGrade, Nobless = player.Nobless, Hero = player.Hero, Subpledge = player.Subpledge, last_recom_date = player.LastRecomDate, lvl_joined_academy = player.LevelJoinedAcademy, Apprentice = player.Apprentice, Sponsor = player.Sponsor, varka_ketra_ally = player.VarkaKetraAlly, clan_join_expiry_time = player.ClanJoinExpiryTime, clan_create_expiry_time = player.ClanCreateExpiryTime, death_penalty_level = player.DeathPenaltyLevel });
            }
            catch (MySqlException ex)
            {
        ...
        public void UpdatePlayer(PlayerModel player)
        {
            try
            {
                db.Execute(@"UPDATE characters SET level=@level,maxHp=@maxHp,curHp=@curHp,maxCp=@maxCp,curCp=@curCp,maxMp=@maxMp,curMp=@curMp,face=@face,hairStyle=@hairStyle,
                                ...
                                death_penalty_level=@death_penalty_level WHERE obj_id=@obj_id", new { level = player.Level, maxHp = player.MaxHp, curHp = player.CurHp, maxCp = player.MaxCp, curCp = player.CurCp, maxMp = player.MaxMp, curMp = player.CurMp, face = player.Face, hairStyle = player.HairStyle, hairColor = player.HairColor, sex = player.Sex, heading = player.Heading, x = player.X, y = player.Y, z = player.Z, exp = player.Exp, expBeforeDeath = player.ExpBeforeDeath, sp = player.Sp, karma = player.Karma, pvpkills = player.PvpKills, pkkills = player.PkKills, clanid = player.ClanId, online = player.Online, onlinetime = player.OnlineTime, race = player.Race, classid = player.ClassId, base_class = player.BaseClass, deletetime = player.DeleteTime, canCraft = player.CanCraft, title = player.Title, rec_have = player.RecHave, rec_left = player.RecLeft, accesslevel = player.AccessLevel, clan_privs = player.ClanPrivs, wantspeace = player.WantsPeace, isin7sdungeon = player.IsIn7sDungeon, punish_level = player.PunishLevel, punish_timer = player.PunishTimer, power_grade = player.PowerGrade, nobless = player.Nobless, subpledge = player.Subpledge, last_recom_date = player.LastRecomDate, lvl_joined_academy = player.LevelJoinedAcademy, apprentice = player.Apprentice, sponsor = player.Sponsor, varka_ketra_ally = player.VarkaKetraAlly, clan_join_expiry_time = player.ClanJoinExpiryTime, clan_create_expiry_time = player.ClanCreateExpiryTime, death_penalty_level = player.DeathPenaltyLevel, obj_Id = player.ObjectId });
            }
            catch (MySqlException ex)
            {
        ...
        public PlayerModel GetPlayerModelBySlotId(string accountName, int slotId)
        {
            try
            {
                return db.Query<PlayerModel>(@"select obj_Id as ObjectId, char_name as Name, Level, MaxHp, CurHp, MaxCp, CurCp,MaxMp,CurMp,
                                                  ...
                                                  varka_ketra_ally as VarkaKetraAlly,clan_join_expiry_time as ClanJoinExpiryTime,clan_create_expiry_time as ClanCreateExpiryTime, death_penalty_level as
                                                  DeathPenaltyLevel from characters where account_name=@account_name AND char_slot=@char_slot", new { account_name = accountName, char_slot = slotId }).FirstOrDefault();
            }
            catch (MySqlException ex)
            {
